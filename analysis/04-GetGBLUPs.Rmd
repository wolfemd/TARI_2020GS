...
predModelA %>% 
  ...
  arrange(desc(Group)) %>% 
  write.csv(., file = here::here("output","GEBV_TARI_ModelA_2020Dec21.csv"), row.names = F)
...
predModelADE %>% 
  ...
  select(Group,GID,any_of(traits)) %>% 
  arrange(desc(Group)) %>% 
  write.csv(., file = here::here("output","GETGV_TARI_ModelADE_2020Dec21.csv"), row.names = F)
```
